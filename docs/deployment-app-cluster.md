...
RAGForge containers read both files through `env_file`, so all three backend replicas receive the same key, JVM settings, Auth Gateway config and webhook secret. Redis is shared on the data layer, so API Key rate limit, JWT revocation state and ShedLock remain consistent across replicas.
