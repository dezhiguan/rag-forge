...
## E-TABLEDOC 表格密集文档检索（10）

| 编号 | 用例名称 | 角色/数据 | 步骤要点 | 预期结果 |
|---|---|---|---|---|
| E-TABLEDOC-01 | xlsx 多 sheet 检索 | / D-XLSX | query 某 sheet 数值 | 命中正确 sheet 的行块 |
| E-TABLEDOC-02 | WHOLE 策略整表成块 | / tablePolicy=WHOLE | 检索 | 整表一块，上下文完整 |
| E-TABLEDOC-03 | ROW 策略逐行成块 | / tablePolicy=ROW | 检索 | 命中精确到行 |
| E-TABLEDOC-04 | 行块携带表头 | / D-DOCX-TABLE | 查 chunk | 每行块带表头语义 |
| E-TABLEDOC-05 | 数值范围 query | / 含数值表 | 检索 | 命中含目标数值的行 |
| E-TABLEDOC-06 | 表格 + 正文混排检索 | / D-DOCX-TABLE | 检索 | 表/文各自正确召回 |
| E-TABLEDOC-07 | csv 大表分块 | / 大 D-CSV | 入库检索 | 按行分批，无超大块，可检索 |
| E-TABLEDOC-08 | 表格命中展示友好 | / D-TABLE | 看结果 | 表格片段结构清晰，非乱码 |
| E-TABLEDOC-09 | 表内跨行关联 query | / 关联两行 | 检索 | 召回相关行（记录跨行局限） |
| E-TABLEDOC-10 | 表格策略 × recall 基线 | / 表格黄金集 | 检索 | WHOLE/ROW 各自 recall 达标并对比 |
...
- 合计 **501** 条（A70 / B81 / C40 / D55 / E90 / F70 / G50 / H45）。
